  ...
  return !isLoading ? (
    <div className='relative px-6 md:px-16 lg:px-40 pt-30 md:pt-40 min-h-[80vh]'>
      ...
      {bookings.map((item , index) =>(
        ...
        <div key={index}
        className='flex flex-col md:flex-row justify-between bg-red-400/8 border-bg-red-400/20 
        rounded-lg mt-4 p-2 max-w-3xl'>
          ...
          <div className='flex flex-col md:items-end md:text-right justify-between p-4'>
           ...
           <p><span className='text-gray-4000'> Total Tickets :</span>{item.bookedSeats.length}</p>
           <p><span className='text-gray-4000'> Seat Number :</span>{item.bookedSeats.join(", ")}</p>
           </div>
          </div>
        </div>
      ))}
